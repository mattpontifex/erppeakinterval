...
%   revised for multipeaks - 3-25-2015 - mp
    ...
    if (r.opt == 1)
        ...
        try, r.Exact; catch, r.Exact = 'False'; end
        try, r.Round; catch, r.Round = 0; end
        ...
        if (r.Latencies == 0)
            % Populate closest latency
            temptimeStart = abs(INERP.times - r.Window(1));
            temptimeStartLat = INERP.times(1,find(temptimeStart == min(temptimeStart)));
            temptimeStop = abs(INERP.times - r.Window(2));
            temptimeStopLat = INERP.times(1,find(temptimeStop == min(temptimeStop)));
            r.Window(1) = temptimeStartLat;
            r.Window(2) = temptimeStopLat;
            ...
            [T, r.Latencies] = evalc('geterpvalues(INERP, r.Window, r.Bin, r.Channels, ''peaklatbl'', ''pre'', 1, r.Polarity, r.Neighborhood, 1, 0.5, 0, 1);'); 
        end
            ...
            for cC = 1:numel(chanlabs)
                if ~isnan(issuelist(cC))
                    ...
                    if (cWin(2) > issueminlat(cC))
                        ...
                        [T, tempWLatencies] = evalc('geterpvalues(INERP, cWin, cBin, cChanI, ''peaklatbl'', ''pre'', 1, cPol, cNei, 1, 0.5, 0, 1);'); 
                        handles2.(sprintf('r%d',cS))(issuelist(cC)).latency = tempWLatencies;
